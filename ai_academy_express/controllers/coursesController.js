...
module.exports = {
    index: (req, res, next) => {
        Course.find({})
          .then(courses => {
            // Assurez-vous de passer `courses` correctement à la vue
            res.render('courses/index', { courses });  // Passer `courses` directement à la vue
          })
          .catch(error => {
      ...
      indexView: (req, res, next) => {
        Course.find({})
          .then(courses => {
            res.render('courses/index', { courses });  // Passer `courses` à la vue
          })
          .catch(error => {
            console.log(`Erreur lors de la récupération des cours: ${error.message}`);
            next(error);
